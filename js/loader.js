...
function Loader(options) {
    // Load a list of stylesheets and/or javascripts and execute a callback
    // once they are all entirely loaded. By default loads files asynchronously,
    // but synchronous loading can be enforced if dependencies are an issue.
    // This loader is entirely native JS 
    var that = this,
        vars = {
            async : true,
            event : 'loaded',
    ...
    for(var k in options) vars[k] = options[k];
...
Loader.prototype.trigger = function(event) {
...
Loader.prototype.listenOnce = function(event, handler) {
    var that = this;
...
Loader.prototype.addListener = function(event, handler) {
    if(!this._listeners[event]) this._listeners[event] = [];
    this._listeners[event].push(handler);
}

Loader.prototype.removeListener = function(event, handler) {
    if(this._listeners[event]) {
...
Loader.prototype.loadFiles = function(files, options) {
    var that = this,
        options = options || {},
        loaded = 0,
        needed = 0,
