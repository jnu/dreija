...
${c.BROWN}WARNINGS${c.NONE}
${c.YELLOW}${[''].concat(json.warnings).join('\n  - ')}${c.NONE}` : '';
...
${c.LIGHT_GREEN}${[''].concat(chunk.files).join(`${c.GREEN}\n    > ${c.LIGHT_GREEN}`)}${c.NONE}`;
    ...
    return `${c.LIGHT_GRAY}${banner}${c.NONE}
...
${c.LIGHT_GRAY}Hiding ${c.CYAN}${otherChunkCount.toLocaleString()}${c.LIGHT_GRAY} other chunks.${errors}${warnings}
${c.LIGHT_GRAY}-------------${c.NONE}
`;
}
